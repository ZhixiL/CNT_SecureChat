...
if __name__ == "__main__":
    ...
    inp = input("Enter text to encrypt (\"Exit\" to quit): ")
    while inp != "Exit":
        random.seed(datetime.now()) #seed the key with current time
        KEY = random.randrange(2**55) #generate key
        enc = encrypt(inp, KEY)
        dec = decrypt(enc, KEY)
        print("Encrypted text: " + enc)
        print("Decrypted Text: " + dec)
        inp = input("Next text (\"Exit\" to quit): ")